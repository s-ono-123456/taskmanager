...
## 2026-09-13 / sqlc生成コードをホストへ書き出す運用を禁止し、代替手順に切り替え

### やったこと

- `internal/taskstore/db.go`（sqlc生成コード）がホストに残っているとユーザーから指摘を受けた。
  原因は、このセッション中に何度も実行した`docker run -v $(pwd)/internal/taskstore:/src
  sqlc/sqlc generate` + `docker run -v $(pwd):/src golang:... go build`という2ステップの
  ビルド確認手順が、ホストディレクトリを書き込み可能でバインドマウントしており、
  生成物がそのままホストへ書き出されていたため。
- 最初は「確認後に削除する」運用でクリーンアップしたが、ユーザーから「仕組み上、生成
  されないようにできないか」「ホスト上で実行するのは禁止してほしい」と重ねて指摘を受け、
  手順そのものを見直した。
- **ビルドが通るかだけの確認**には`docker build -f build/Dockerfile -t task-dashboard:verify .`
  を使うよう変更した。`build/Dockerfile`のsqlc-genステージは`COPY`（バインドマウントでは
  ない）でソースを取り込んでいるため、構造的にホストへ生成物が出力されない。実際に
  ビルドが通ること・ホストに生成物が一切出ないことを確認済み。
- **`go vet`/`go test`等、ビルド以外のGoコマンドが必要な場合**は、ホストのソースを
  読み取り専用（`:ro`）でマウントし、Docker named volumeへコピーしてから、
  以降の生成・ビルド・vet・testをすべてvolume内で行う手順に変更した（volumeはDockerの
  管理領域であり、リポジトリのワーキングディレクトリには存在しない）。この手順も
  `sqlc generate`→`go build`→`go vet ./...`まで通しで動作すること、ホストに一切ファイルが
  残らないことを実機で確認済み。
- `docs/design/design.md`「開発時のビルド方法」・`README.md`「実行方法」を新しい手順に
  書き換え、旧手順（ホストへ書き込み可能なバインドマウント）は「禁止・参考のみ」として
  明示的に使用禁止と書いた。

### 学んだこと

- 「ローカルに生成しない」という運用ルールをドキュメントに書いていても、検証のたびに
  手動でクリーンアップする運用は、確認漏れが起きる前提で設計されていない限りいずれ
  破られる。ルール違反が構造的に起こり得ない手段（バインドマウントではなくCOPY、
  または書き込み専用のnamed volume）に置き換えることで、初めて「今後同じ問題が起きない」
  と言える。ユーザーからの「仕組み上どうにかできないか」という指摘は、運用ルールの
  遵守を毎回の注意力に頼っている箇所を見直す良いシグナルだった。
